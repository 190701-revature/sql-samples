...
--DML - Used for manipulating table data
...
--		A sequence is stateful database object that represents
--		a sequence of values.
--  3. Sets a default value for the column, to pull from the 
--		sequence. 
...
-- Constraints
-- 1. Primary Key - Unique Identifier for a row, implicitly
--				unique and not null
-- 2. Foreign Key - Reference to other row by a foreign key value,
--				generally the target table's primary key column.
-- 3. NOT NULL - Null values are invalid for this column.
-- 4. unique - Every value in the unique column or columns must not
--				be repeated in later columns.
-- 5. Check - Column value must return true when provided to a 
--				predicate function.
--     ex) my_col INTEGER CHECK (my_col % 2 = 0)
-- 6. Exclude - Not commonly used, requires some external support.
--			Given a comparison rule and compared with all other column
